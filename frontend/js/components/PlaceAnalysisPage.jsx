...
window.PlaceAnalysisPage = function PlaceAnalysisPage(props) {
    ...
    var sbizWhy = function(m) { return (m && !m.sbiz && m.sbizReason) ? (SBIZ_WHY[m.sbizReason] || null) : null; };
                ...
                (!sb && sbizWhy(m))
                    ? React_.createElement('div', { className: 'card', style: { marginTop: 14 } },
                        ...
                        React_.createElement('div', { className: 'note est', style: { marginTop: 10 } },
                            React_.createElement('b', null, '이 회차에는 상권 자료가 붙지 않았습니다'), ' — ', sbizWhy(m)))
                    : null,
                // 동네 상권
                sb ? React_.createElement('div', { className: 'card', style: { marginTop: 14 } },
                        ...
                        sbizIsGu(sb)
                            ? React_.createElement('div', { className: 'note est', style: { marginBottom: 10 } },
                                ...
                                (sb.guReason === 'dong-no-stats'
                                    ? ' — 이 동네에는 이 업종의 공공 통계가 없어 한 단계 넓은 범위로 봅니다. 동네는 정확히 찾았습니다.'
                                    : ' — 적어주신 지역을 동 단위로 특정하지 못해 범위를 넓혔습니다. 동 단위로 보려면 시·구까지 적어주세요(예: 고양시 성사동).'))
                            : null,
                        React_.createElement('div', { className: 'grid3' },
                            React_.createElement('div', { className: 'ratecard p' },
