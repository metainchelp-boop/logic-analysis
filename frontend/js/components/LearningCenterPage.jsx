 ...
 * 광고센터(업체관리)의 광고주 데이터를 기반으로, 각 광고주별 AI 자기학습 인사이트
...
window.LearningCenterPage = function LearningCenterPage({ currentUser }) {
    ...
    var statusMeta = {
        active: { label: '진행중', color: '#16a34a', bg: '#dcfce7' },
        paused: { label: '일시중지', color: '#ca8a04', bg: '#fef9c3' },
        terminated: { label: '종료', color: '#dc2626', bg: '#fee2e2' },
    };

    /* ==================== 광고주 사이드바 아이템 ==================== */
    var renderClientItem = function(c) {
        var isActive = selectedClient && selectedClient.id === c.id;
        var sm = statusMeta[c.status] || statusMeta.active;
        return React.createElement('button', {
            ...
            React.createElement('div', { style: { display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 6 } },
                React.createElement('span', { style: { fontSize: 13, fontWeight: isActive ? 700 : 500, color: isActive ? '#6d28d9' : '#334155', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' } }, c.name),
                React.createElement('span', { style: { flexShrink: 0, fontSize: 10, fontWeight: 600, padding: '1px 7px', borderRadius: 8, background: sm.bg, color: sm.color } }, sm.label)
            ),
    ...
    var renderRight = function() {
        if (!selectedClient) {
            return React.createElement('div', { className: 'card', style: { padding: 48, textAlign: 'center', color: '#94a3b8' } },
                ...
                React.createElement('div', { style: { fontSize: 13, marginTop: 8, lineHeight: 1.7 } }, '선택한 광고주의 누적 분석 데이터를 바탕으로\nAI가 학습한 인사이트(성과 패턴·경쟁사 감지·가격/광고/순위 예측 등)를 보여줍니다.')
            );
        }
