...
function rankCellLabel(r) {
    ...
    const [vertical, setVertical] = useState(function() {
        try { return localStorage.getItem('cdVertical') || 'all'; } catch (e) { return 'all'; }
    });
    var setVerticalKeep = function(v) {
        setVertical(v);
        try { localStorage.setItem('cdVertical', v); } catch (e) {}
    };
    ...
    var isPlaceClient = function(c) { return (c && c.vertical) === 'place'; };
            ...
            <div className="container cd-layout" style={{ display: 'flex', gap: 20, paddingTop: 20, minHeight: 'calc(100vh - 80px)' }}>
                ...
                <div className="cd-sidebar" style={{ width: 280, flexShrink: 0 }}>
                    <div className="card" style={{ padding: 16 }}>
                        <div style={{ fontSize: 16, fontWeight: 700, marginBottom: showVerticalTabs ? 8 : 12 }}>
                            {(currentUser && currentUser.role === 'viewer') ? '내 영업 대상' : '내 업체 목록'} ({byVertical.length})
                        </div>
                        {showVerticalTabs && (
                            <div style={{ display: 'flex', gap: 4, marginBottom: 10, background: '#f1f5f9', borderRadius: 8, padding: 3 }}>
                                {[
                                    { k: 'all', label: '전체', n: clients.length },
                                    { k: 'store', label: '🛒 스토어', n: storeCount },
                                    { k: 'place', label: '📍 플레이스', n: placeCount }
                                ].map(function(t) {
                                    var on = effVertical === t.k;
                                    return (
                                        <button key={t.k} type="button"
                                            onClick={function() { setVerticalKeep(t.k); }}
                                            title={t.k === 'store' ? '스마트스토어 — 쇼핑 순위'
                                                 : (t.k === 'place' ? '스마트플레이스 — 지도 순위' : '스토어·플레이스 전부')}
                                            style={{
                                                flex: 1, border: 0, cursor: 'pointer', borderRadius: 6,
                                                padding: '6px 4px', fontSize: 11.5, fontWeight: 800,
                                                background: on ? '#fff' : 'transparent',
                                                color: on ? '#1d4ed8' : '#64748b',
                                                boxShadow: on ? '0 1px 3px rgba(15,23,42,.12)' : 'none',
                                                whiteSpace: 'nowrap'
                                            }}>
                                            {t.label} <span style={{ fontVariantNumeric: 'tabular-nums', opacity: .75 }}>{t.n}</span>
                                        </button>
                                    );
                                })}
                            </div>
                        )}
                        {/* ⚠️ 검색칸 노출은 **고른 종류가 아니라 전체 개수**로 판단한다.
                            종류별 개수로 하면 탭을 옮기는 순간 칸이 사라지는데, 이미 친 검색어는
                            그대로 걸려 있어 「검색 결과가 없습니다」만 뜨고 지울 방법이 없어진다. */}
                        {clients.length > 3 && (
                            <input className="form-input"
                                placeholder={effVertical === 'place' ? '플레이스 업체 검색...'
                                           : (effVertical === 'store' ? '스토어 업체 / 키워드 검색...' : '업체명 / 키워드 검색...')}
                                value={searchQuery}
                        ...
                        {!loading && clients.length > 0 && filteredClients.length === 0 && (
                            <div style={{ color: '#94a3b8', fontSize: 13, padding: '20px 0', textAlign: 'center' }}>
                                {byVertical.length === 0
                                    ? <span>{effVertical === 'place' ? '플레이스' : '스토어'} 업체가 없습니다.<br />
                                        <button type="button" onClick={function() { setVerticalKeep('all'); }}
                                            style={{ marginTop: 8, border: 0, background: '#eff6ff', color: '#1d4ed8', fontWeight: 800, fontSize: 12, borderRadius: 6, padding: '5px 10px', cursor: 'pointer' }}>
                                            전체 보기
                                        </button></span>
                                    : '검색 결과가 없습니다.'}
                            </div>
