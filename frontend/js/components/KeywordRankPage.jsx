 ...
 * ─────────────────────────────────────────────────────────────────────── */
var _krHvLogin = { 'in': '로그인', out: '로그아웃', unknown: '미확인' };
function _krHvPct(v) { return (v === null || v === undefined) ? '미확인' : Math.round(v * 100) + '%'; }
function HumanViewCard(props) {
    var user = props.currentUser || {};
    var isSuper = user.role === 'superadmin';
    var st = React.useState({ data: null, error: '', token: '', tokenMsg: '' });
    var view = st[0], setView = st[1];
    var alive = React.useRef(true);
    function load() {
        Promise.resolve().then(function() { return api.get('/human-view/stats'); }).then(function(res) {
            if (!res || !res.success) throw new Error('bad');
            if (alive.current) setView(function(v) { return Object.assign({}, v, { data: res, error: '' }); });
        }).catch(function() {
            if (alive.current) setView(function(v) { return Object.assign({}, v, { error: '읽기 도우미 기록을 읽지 못했습니다(미확인).' }); });
        });
    }
    React.useEffect(function() {
        alive.current = true;
            ...
            : React.createElement('div', { style: { display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 8, marginTop: 10 } },
                pcs.map(function(p) {
                    return React.createElement('div', { key: p.pc }, _krOpsTile('PC ' + p.pc + '번', _krOpsNum(p.keywords) + '개 키워드',
                        '화면 ' + _krOpsNum(p.uploads) + ' · 완료 ' + _krOpsNum((p.complete || 0) + (p.allFound || 0)) + ' · 마지막 ' + _krOpsClock(p.lastAt)));
                }))),
        d && recent && recent.length > 0 && React.createElement('div', { style: { overflowX: 'auto', marginTop: 10 } },
            React.createElement('table', { style: { width: '100%', borderCollapse: 'collapse' } },
                ...
                React.createElement('tbody', null, recent.map(function(r, i) {
                    ...
                    return React.createElement('tr', { key: i },
                        ...
                        React.createElement('td', { style: _krTd }, (_krHvLogin[r.loggedIn] || r.loggedIn) + (r.recorded ? '' : ' · 대조만')),
                        React.createElement('td', { style: _krTd }, _krOpsClock(r.at)));
                })))),
        d && lc && React.createElement('p', { style: { fontSize: 12, color: '#475569', margin: '8px 0 0' } },
            '첫 주 대조(최근 7일 · 같은 날 수집기 결과와 앞 20개가 같은 자리) — 로그인 화면 ' + _krOpsNum(lc['in'] && lc['in'].pairs) + '건 ' + _krHvPct(lc['in'] && lc['in'].samePos) +
            ' · 로그아웃 화면 ' + _krOpsNum(lc.out && lc.out.pairs) + '건 ' + _krHvPct(lc.out && lc.out.samePos) +
...
window.KeywordRankPage = function KeywordRankPage(props) {
    ...
    var _rgN = useState(''); var regNv = _rgN[0], setRegNv = _rgN[1];
    var _rgNB = useState(false); var nvBusy = _rgNB[0], setNvBusy = _rgNB[1];
    var _rgNM = useState(null); var nvMsg = _rgNM[0], setNvMsg = _rgNM[1];   // {ok, text}
    ...
    var lookupNv = function() {
        var u = (regUrl || '').trim();
        if (!u || nvBusy) return;
        setNvBusy(true); setNvMsg(null);
        var kws = (regKw || '').split(',').map(function(k) { return k.trim(); }).filter(Boolean);
        api.post('/products/nvmid-lookup', { product_url: u, keywords: kws })
           .then(function(res) {
               var d = (res && res.data) || {};
               if (d.found && d.nv_mid) {
                   ...
                   setNvMsg({ ok: false, text: d.message || '찾지 못했습니다 — nvMid 를 직접 넣어 주세요.' });
               }
           })
           .catch(function(e) {
               setNvMsg({ ok: false, text: '자동 찾기 실패 — ' + ((e && e.message) || '네트워크 오류')
                                          + '. nvMid 를 직접 넣어 주세요.' });
           })
    ...
    var nvOk = function(v) { var t = cleanNv(v); return t.length >= 8 && t.length <= 20; };
    ...
    var submitRegister = function() {
        ...
        api.post('/products/track', {
            ...
            setRegUrl(''); setRegKw(''); setRegNv(''); setNvMsg(null);
    ...
    return React.createElement('div', { style: _krWrap },
            ...
            if (trayTab === 'stuck') {
                ...
                body = [React.createElement('div', { key: 'n', style: { margin: '10px 14px 4px', padding: '8px 12px', borderRadius: 7, fontSize: 12.5, background: '#fffbeb', border: '1px solid #fde68a', color: '#92400e' } },
                    ...
                    .concat(shelved.map(function(pd) {
                        ...
                        return React.createElement('div', { key: pd.id, style: rowBase },
                            React.createElement('span', { style: { fontWeight: 600, flex: '1 1 220px', minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }, title: pd.name }, pd.name || '(이름 없음)'),
                            pd.store && React.createElement('span', { style: { color: '#64748b', fontSize: 12, maxWidth: 140, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }, title: '스토어: ' + pd.store }, '🏬 ' + pd.store),
                            React.createElement('span', { style: { flex: '1 1 140px', color: '#94a3b8', fontSize: 12, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }, title: pd.keywords.join(', ') }, pd.keywords.join(' · ') || '—'),
                            ...
                            sugs.length > 0 && React.createElement('span', { style: { display: 'inline-flex', alignItems: 'center', gap: 5, flexWrap: 'wrap' } },
                                React.createElement('span', { style: { fontSize: 11, color: '#94a3b8' } }, '추정 업체'),
                                sugs.map(function(s) {
                                    return s.eligible
                                        ? React.createElement('button', {
                                            key: s.id, disabled: trayBusy === pd.id,
                                            onClick: function() { trayAct(pd.id, '/relink', { client_id: s.id }, '연결하고 되살렸습니다'); },
                                            title: '누르면 이 업체로 연결되고 그 자리에서 되살아납니다',
                                            style: { fontSize: 11, fontWeight: 700, padding: '2px 9px', borderRadius: 99, cursor: 'pointer',
                                                     background: '#eff6ff', color: '#1d4ed8', border: '1px solid #bfdbfe' }
                                        }, '↩ ' + s.name + ' 연결')
                                        : React.createElement('span', {
                                            key: s.id, title: '계약이 끝난 업체라 연결해도 살아나지 않습니다',
                                            style: { fontSize: 11, padding: '1px 8px', borderRadius: 99,
                            ...
                            actBtn(open ? '연결 취소' : '업체 연결', !canRevive,
                                function() { setLinkFor(open ? null : pd.id); setLinkQ(open ? '' : (pd.store || '')); setLinkOpts([]); }),
        ...
        !selected && canEditHere && React.createElement('div', { style: _krRegBox },
            ...
            React.createElement('div', { style: _krRegRow },
                ...
                React.createElement('div', null,
                    React.createElement('label', { style: _krRegLbl }, 'nvMid ', React.createElement('span', { style: { color: '#ef4444' } }, '*')),
                    React.createElement('div', { style: { display: 'flex', gap: 6 } },
                        React.createElement('input', {
                            style: Object.assign({}, _krRegInp, { flex: 1, minWidth: 0 }),
                            value: regNv, placeholder: '숫자만 · 예 84321234',
                            onChange: function(e) { setRegNv(e.target.value); setNvMsg(null); },
                            onBlur: function(e) { setRegNv(cleanNv(e.target.value)); },
                            onKeyDown: function(e) { if (e.key === 'Enter') submitRegister(); }
                        }),
                        React.createElement('button', {
                            onClick: lookupNv, disabled: !regUrl.trim() || nvBusy,
                            title: '이미 모아 둔 수집분에서 찾습니다 — 네이버에 요청하지 않습니다',
                            style: { border: '1px solid #bfdbfe', background: '#fff', color: '#1d4ed8',
            ...
            nvMsg && React.createElement('div', {
                style: { fontSize: 11, marginTop: 7, fontWeight: 700,
                         color: nvMsg.ok ? '#047857' : '#b45309' }
            }, (nvMsg.ok ? '✅ ' : '⚠️ ') + nvMsg.text),
                ...
                regMsg ? regMsg.text
                       : (regClient
                            ...
                            : (regQuery.trim() && regMeta && regOpts.length === 0
                                ...
                                : (regClient === null && !regQuery.trim() && !nvOk(regNv) && regUrl.trim()
                                    ? 'nvMid 를 넣어야 등록됩니다 — 「🔎 자동 찾기」를 눌러 보세요(네이버 요청 없음).'
