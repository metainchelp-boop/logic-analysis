    ...
    return (
        <div className="section fade-in" id="sec-seo">
            <div className="container">
                ...
                {/* ========== 상세페이지 진단 분석 ========== */}
                <DetailPageAnalysis productUrl={productUrl} keyword={keyword} seoResult={result} />
...
/* ========== 상세페이지 진단 분석 서브 컴포넌트 ========== */
window.DetailPageAnalysis = function DetailPageAnalysis({ productUrl, keyword, seoResult }) {
    ...
    // SEO 분석 결과가 나온 후 자동 실행
    useEffect(function() {
        if (seoResult && productUrl && !autoTriggered.current && !dpResult && !dpLoading) {
            autoTriggered.current = true;
            runDetailAnalysis();
        }
    }, [seoResult, productUrl]);
    ...
    const scoreColor = (s) => s >= 70 ? '#059669' : s >= 40 ? '#d97706' : '#dc2626';
    const scoreBg = (s) => s >= 70 ? '#ecfdf5' : s >= 40 ? '#fffbeb' : '#fef2f2';
    const scoreLabel = (s) => s >= 70 ? '우수' : s >= 40 ? '보통' : '미흡';
                ...
                <div className="fade-in">
                    ...
                    <div className="card" style={{ textAlign: 'center', marginBottom: 16, padding: '20px', background: scoreBg(dpResult.scores.total) }}>
                        ...
                        <div style={{ marginTop: 6, fontSize: 13, fontWeight: 700, color: scoreColor(dpResult.scores.total) }}>
                            {scoreLabel(dpResult.scores.total)}
                        </div>
                    </div>
