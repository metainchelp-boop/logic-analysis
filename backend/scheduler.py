...
def start_scheduler():
    ...
    # 8) 주간 로직 분석 보고서 — 매주 월요일 09:40 (08:30 분석 잡이 끝난 뒤).
    #    ① 메타 전산이 광고주 공유 대시보드에 누적해 보여준다(2026-08-20 합의).
    #    ⚠️ 새 분석을 돌리지 않는다 — 그 주 client_analyses.report_html 을 골라 등록만 한다.
    #    ⚠️ 08:30 잡의 실제 소요 시간을 아직 못 쟀다(로그 미확보). 겹치면 이 시각을 뒤로
    #       미루고 ①에 통지할 것. max_instances=1 이라 겹쳐도 중복 실행은 안 된다.
    #    생성 직후 같은 잡 안에서 보관정책(12주)까지 돈다.
    _scheduler.add_job(
        _run_weekly_reports,
    ...
    # 소상공인365 상권 API 자가 점검 — 매일 05:00 (04:00 계약동기화 뒤, 08:00 배치 전 한산한 시각).
    # ⚠️ 이게 있어야 아무도 안 써도 죽은 걸 안다. 직원 호출 기록만으로는
    #    「아무도 안 썼다」와 「죽었다」가 구분되지 않는다(2026-08-30 실사고).
    _scheduler.add_job(
        _run_sbiz_health_probe,
        trigger=CronTrigger(hour=5, minute=0),
        id="sbiz_health_probe",
        name="소상공인365 자가 점검 (05:00)",
        replace_existing=True,
        max_instances=1,
    )
...
def _run_sbiz_health_probe():
    """상권 API 가 살아 있는지 하루 한 번 스스로 찔러 본다 (대표 확정 2026-08-30, 안 「나」).

    ⚠️ 왜 필요한가 — 직원 호출 기록만으로는 **「아무도 안 썼다」와 「죽었다」를 못 가른다.**
       2026-08-30 에 상권 API 가 11:50~12:24 죽어 있었는데, 그 사이 아무도 안 돌려서
       로그에 `[sbiz365]` 가 한 줄도 없었다 — 「언제부터냐」에 답할 방법이 아예 없었다.

    하루 외부 호출 1회(표본 중 하나라도 되면 즉시 중단)라 부담이 없다.
    ⚠️ 실패해도 예외를 밖으로 내지 않는다 — 점검이 다른 배치를 넘어뜨리면 안 된다.
    """
    try:
        from sbiz_health import run_probe
        r = run_probe()
        if r.get("ok"):
...
def _run_rank_tracking():
    ...
    try:
        ...
        try:
            from rank_link import share_targets   # 나눠 적을 곳을 고르는 규칙(순수 함수)
...
# ==================== 주간 로직 분석 보고서 (월 09:40) ====================

def _run_weekly_reports():
    """활성 광고주마다 주 1건 보고서를 reports 표에 등록 + 12주 보관정책.

    구현은 weekly_report.py — 여기서는 스케줄러 진입점만 둔다.
    실패해도 다른 배치에 영향이 없도록 통째로 감싼다.
    """
    try:
        from weekly_report import run_weekly_reports
        run_weekly_reports()
    except Exception as e:
