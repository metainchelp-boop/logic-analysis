...
# 2026-09-23 대표 지시 「상한을 15로 맞추면 될 거 같은데. 위험 부담도 크지 않을거 같고」 → **10 → 15**.
#   실측 근거(9/23 19시): 유니버스 594개인데 10 이면 두 대 합쳐 시간당 20·하루 480 이라 한 바퀴를 못 돈다.
#   15 면 시간당 30·하루 720 으로 전량 + 여유. 요청 속도는 기계(=회선)당 시간당 최대 15×8장 = 120건으로
#   2026-08-05 차단 때(한 회선 시간당 약 1,272건)의 1/10 아래다. 두 대는 서로 다른 회선이다.
#   ⚠️ 온디맨드(밀린 요청 큐)는 이 값이 0 보다 크면 여전히 안 나간다 — 큐 정리는 별건.
DEFAULT_TEST_CAP = 15
