...
def test_naver_store_slug_requires_exact_supported_host():
    """#246 스마트·브랜드스토어 슬러그만 읽고 유사/악성 호스트는 거부한다."""
    supported = {
...
def test_review_count_api_is_smartstore_only():
    """#246 브랜드 URL을 스마트스토어 내부 리뷰 API로 바꿔 호출하지 않는다."""
    calls = []
...
def test_smartstore_detail_api_skips_brand_url():
    """#246 스마트스토어 JSON API 폴백도 브랜드 URL에는 호출되지 않는다."""
    calls = []
    original_get = nc.requests.get
...
def test_brand_store_preloaded_detail_data():
    """#246 브랜드스토어 캡처에서도 가격·카테고리·리뷰·평점·스토어를 보존한다."""
    html = r'''
    ...
    rd = (result or {}).get("reviewData") or {}
    assert rd.get("price") == 12300, rd
    assert rd.get("category") == "생활/건강>반려동물>강아지 간식>동결건조 간식", rd
    assert rd.get("category1") == "생활/건강", rd
    assert rd.get("reviewCount") == 540, rd
    assert rd.get("rating") == 4.88, rd
    assert (result.get("storeInfo") or {}).get("name") == "Vaya", result.get("storeInfo")


def test_smartstore_next_data_detail_data_unchanged():
    """#246 폴백 추가 후에도 기존 스마트스토어 NEXT_DATA를 우선 보존한다."""
    html = '''
        <!DOCTYPE html><html><head>
    ...
    rd = (result or {}).get("reviewData") or {}
    assert rd.get("price") == 9900, rd
    assert rd.get("category") == "식품>과자>쿠키", rd
    assert rd.get("category1") == "식품", rd
