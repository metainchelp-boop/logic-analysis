...
def _resolve_admi(region: str, biz_name: str = ""):
    ...
    if (biz_name or "").strip():
        ...
        # ⭐ 업체명 단독도 물어본다(2026-08-20 실측) — 상호에 지역명이 없으면
        #    지역검색이 「{업체명} {지역}」을 0건으로 돌려준다.
        #    실측: '프로힐링지압안마원 조원동' → 0건 / '프로힐링지압안마원' → 1건
        #          (주소 '경기도 수원시 장안구 조원동 894' = 우리가 찾던 바로 그 동네).
        #    ⚠️ 업체명만 물으면 동명 업체가 잡힐 수 있으나, 바로 아래 주소 대조가
        #       「입력 지역과 같은 동네인가」를 다시 확인하므로 엉뚱한 동네는 통과 못 한다.
        #       그래서 지역 시설 질의보다 **앞**에 둔다(업체 주소가 가장 정확한 근거).
        queries.append(biz_name.strip())
...
#
# ⚠️ **「상권 API 를 부르지도 않은 것」은 기록하지 않는다.** 그것까지 실패로 세면
#    「며칠째 죽었다」가 늘 참이 돼 신호가 죽는다. 아래 사유는 전부 `_fetch_avg` 이전에 끊긴다:
#      no-key            키 미설정          — 모듈 자체가 꺼진 상태
#      no-region/-industry  입력이 비었다    — 직원이 업종을 안 고른 것
#      region-unresolved 지역 판정 실패     — ⚠️ 여기서 쓰는 건 **네이버 지역검색**이지
#                                            소상공인365 가 아니다. 상권 API 는 손도 안 댔다.
#
# ⚠️ region-unresolved 를 2026-08-30 에 뒤늦게 넣었다. 그날 배포한 자가 점검의 표본
#    첫 곳(성수동)이 **상시** 이 사유로 끊긴다는 걸 배포 뒤 실측으로 알았다
#    (주소가 '성동구'로만 와서 동 이름 대조에 걸린다). 그대로 뒀으면 API 가 멀쩡한 날에도
#    매일 실패가 한 건씩 쌓여, 정작 진짜 장애 때 그 신호가 묻혔을 것이다.
#    ⇒ **경보를 만들 때는 「정상인 날에 무엇이 기록되는가」를 먼저 재 볼 것.**
#
# ⚠️ industry-unmapped 는 일부러 남겨 둔다 — 업종 트리는 소상공인365 에서 받아 오므로
#    (30일 캐시) 그 API 가 죽으면 이 사유로 나타날 수 있다. 애매하지만 API 쪽이다.
_SBIZ_NOT_TRIED = {"no-key", "no-industry", "no-region", "region-unresolved"}
