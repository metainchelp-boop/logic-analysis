...
def _get_claude_client() -> Optional[anthropic.Anthropic]:
    """Anthropic 클라이언트를 싱글톤으로 재사용 (매 요청마다 생성하지 않음)"""
    global _claude_client
    if not CLAUDE_API_KEY:
        return None
    if _claude_client is None:
        _claude_client = anthropic.Anthropic(api_key=CLAUDE_API_KEY)
        logger.info("[Chat] Anthropic 클라이언트 초기화 완료")
...
def _fetch_context_data(message: str) -> str:
    ...
    try:
        ...
        logger.error(f"[Chat] RAG 데이터 조회 오류: {e}", exc_info=True)
    finally:
        conn.close()
...
def _call_claude_sync(messages: list, user_message: str, context_data: str = "",
    ...
    try:
        ...
        if resp.content:
            logger.info(f"[Chat] Claude 응답 완료 (토큰: input={resp.usage.input_tokens}, output={resp.usage.output_tokens})")
            return resp.content[0].text
        return "응답을 처리할 수 없습니다."

    except anthropic.APITimeoutError:
        logger.warning("[Chat] Claude API 타임아웃")
        return "AI 응답 시간이 초과되었습니다. 잠시 후 다시 시도해주세요."
    except anthropic.RateLimitError:
        logger.warning("[Chat] Claude API 속도 제한")
        return "AI 요청이 많아 잠시 대기 중입니다. 30초 후 다시 시도해주세요."
...
async def send_message(req: ChatRequest, current_user: dict = Depends(get_current_user)):
    ...
    try:
        ...
        return {"success": False, "detail": "메시지 처리 중 오류가 발생했습니다."}
    finally:
        conn.close()
